...
#if defined(NDEBUG)
# error "Energon cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xcad0d37d4c900bf243cbfffb40875f2b80b220d25340a55a6a1664de3593a29c");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Energon: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Energon Signed Message:\n";
...
{
    // Energon: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Energon: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 30; // Energon: 5 minutos
static const int64 nTargetSpacing = 1 * 30; // Energon: 30 segundos
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Energon: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Energon: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xcad0d37d4c900bf243cbfffb40875f2b80b220d25340a55a6a1664de3593a29c");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Esto es energon, energia de autobots";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0xfe48a6ff3b4911a912d0879b13461d8da57d5d72da7674d35c08f1e948203d20"));
/// lo adicionado
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Energon: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("EnergonMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("EnergonMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static EnergonMiner(CWallet *pwallet)
{
    printf("EnergonMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("energon-miner");
    ...
    try { loop {
        ...
        printf("Running EnergonMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("EnergonMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&EnergonMiner, pwallet));
}
